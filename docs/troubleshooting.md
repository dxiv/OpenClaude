...
- **`DEIMOS_DEBUG_LOG_FILE`** (preferred) overrides the default debug log **file** path when **`--debug-file`** is not set. **`DEIMOS_DEBUG_LOGS_DIR`** is the legacy name for the same behavior (full path to a **file**, not a directory — see **`getDebugLogPath()`** in `src/utils/debug.ts`). If both are set, **`DEIMOS_DEBUG_LOG_FILE`** wins. **`--debug-file`** wins over either env var when present.
